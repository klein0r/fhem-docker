#########################################################################################################################
# $Id: 76_SMAPortal.pm 23165 2020-11-15 20:43:06Z DS_Starter $
#########################################################################################################################
#       76_SMAPortal.pm
#
...
sub setVersionInfo {
  ...
  if($modules{$type}{META}{x_prereqs_src} && !$hash->{HELPER}{MODMETAABSENT}) {
      ...
      if($modules{$type}{META}{x_version}) {                                                                             # {x_version} ( nur gesetzt wenn $Id: 76_SMAPortal.pm 23165 2020-11-15 20:43:06Z DS_Starter $ im Kopf komplett! vorhanden )
          $modules{$type}{META}{x_version} =~ s/1\.1\.1/$v/gx;
      } 
      else {
          $modules{$type}{META}{x_version} = $v; 
      }
      return $@ unless (FHEM::Meta::SetInternals($hash));                                                                # FVERSION wird gesetzt ( nur gesetzt wenn $Id: 76_SMAPortal.pm 23165 2020-11-15 20:43:06Z DS_Starter $ im Kopf komplett! vorhanden )
